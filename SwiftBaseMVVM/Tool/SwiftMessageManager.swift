...
class SwiftMessageManager: NSObject {
    ...
    class func showMessage(layoutType:LayoutType, themeType :ThemeType ,iconImageType : IconStyle, presentationStyleType :SwiftMessages.PresentationStyle, title :String,body:String, isHiddenBtn: Bool,seconds :TimeInterval) {
        ...
        var view = MessageView.viewFromNib(layout: MessageView.Layout.messageView)
        ...
        case .CardView:
            view = MessageView.viewFromNib(layout: MessageView.Layout.cardView)
            
        case .TabView:
            view = MessageView.viewFromNib(layout: MessageView.Layout.tabView)
            
        case .StatusLine:
            view = MessageView.viewFromNib(layout: MessageView.Layout.statusLine)
            
        default:
            
            view = MessageView.viewFromNib(layout: MessageView.Layout.messageViewIOS8)
        }
